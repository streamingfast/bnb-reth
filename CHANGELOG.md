...
- Initial Firehose instrumentation on top of bnb-chain/reth, ported from
  streamingfast/reth `firehose/2.x` (`v2.3.0-fh-6`). Includes the `reth-firehose` crate
  (inspector, block tracer, executor wrappers, ExEx), live engine-tree tracing in the
  payload validator, and pipeline (staged sync) tracing.
